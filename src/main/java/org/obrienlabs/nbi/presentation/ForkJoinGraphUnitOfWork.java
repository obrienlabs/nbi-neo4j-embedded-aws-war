...
public class ForkJoinGraphUnitOfWork extends RecursiveAction {
    ...
    private String port;
    private String pass; 
    private static long batchSize = 512;
    private int threadMult;
	
	public ForkJoinGraphUnitOfWork(long start, long _end, long _length, String pass, String port, String ip, int threadMult) {
		startId = start;
		length = _length;	
		endId = _end;
		this.ip = ip;
	...
	protected void compute() {
	    ...
	    // recursive case
	    invokeAll(
	    		new ForkJoinGraphUnitOfWork(startId, startId + split, split, pass, port, ip, threadMult),
	    		new ForkJoinGraphUnitOfWork(startId + split, endId, split, pass, port, ip, threadMult)
	    );		
	}
}
